...
pub(crate) fn max_h3_data_payload_for_capacity(cap: usize) -> usize {
    ...
    // payload + overhead(payload) <= cap
    let mut payload = cap.saturating_sub(HTTP3_DATA_FRAME_TYPE_WIRE_LENGTH + 1);
    while payload > 0 {
        let need = http3_data_frame_overhead(payload) + payload;
        if need <= cap {
            return payload;
        }
        let overflow = need - cap;
        payload = payload.saturating_sub(overflow.max(1));
    }
    0
}
...
mod h3_frame_tests {
    ...
    fn max_payload_fits_capacity() {
        for cap in [0usize, 1, 2, 3, 10, 100, 1500, 65535] {
            let p = max_h3_data_payload_for_capacity(cap);
            if p == 0 {
                continue;
            }
            assert!(http3_data_frame_overhead(p) + p <= cap);
